...
public class MemberModel {
  ...
  @RequestMapping("member/idcheck_result.do")
	  public String member_idcheck_result(HttpServletRequest request,HttpServletResponse response) {
		...
		request.setAttribute("count", count); //JSP로 전송
  ...
  public String member_login(HttpServletRequest request,HttpServletResponse response) {
	 // header에 있는 data:{"id":id, "pwd":pwd},
	  ...
	  if(vo.getMsg().equals("OK")) { // 로그인 되었다면
		  ...
		  // session 생성(session, cookie는 request를 통해 생성)
  ...
  public void member_idfind2_ok(HttpServletRequest request,HttpServletResponse response) {
	  String email = request.getParameter("email");
	  MemberDAO dao = new MemberDAO();
	  String res = dao.memberIdfind(email);
	  try {
		PrintWriter out = response.getWriter();
		out.println(res);
  ...
  // => 회원 수정, 회원 탈퇴 => session(이름변경해서 저장), session을 해제
