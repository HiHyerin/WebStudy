...
<div class="wrapper row3">
  <main class="container clear"> 
					...
					<c:forEach var = "rvo" items="${list }">
						<table class="table">
						  ...
						  <tr id="r${rvo.rno }" class="rinsert" style = "display:none">
						  	<td colspan="2">
						  		<form method = "post" action="../freeboard/reply_reply_insert.do"> <!--  모델로 고고 -->
									<input type=hidden name= "bno" value="${vo.no }">
									<input type=hidden name= "pno" value="${rvo.rno }">
									<textarea rows="3" cols="90" name="msg" style="float: left"></textarea> &nbsp;
									<input type=submit value="댓글" class="btn btn-sm btn-danger" style="height: 60px">
								</form>
						  	</td>
						  </tr>
						  ...
						  <tr id="u${rvo.rno }" class="rupdate" style = "display:none">
						  	<td colspan="2">
						  		<form method = "post" action="../freeboard/reply_update.do"> <!--  모델로 고고 -->
									<input type=hidden name= "bno" value="${vo.no }">
									<input type=hidden name="rno" value="${rvo.rno }">
									<textarea rows="3" cols="90" name="msg" style="float: left">${rvo.msg }</textarea> &nbsp;
									<input type=submit value="수정" class="btn btn-sm btn-danger" style="height: 60px">
								</form>
						  	</td>
						  </tr>
						</table>
	...
	<c:if test="${sessionScope.id != null }"> <!-- 댓글 쓰는 위치(로그인이 되어야만 쓸 수 있게 할 것임) -->
		<table class="table">
			<tr>
				<td>
					<form method = "post" action="../freeboard/reply_insert.do"> <!--  모델로 고고 -->
						<input type=hidden name= "bno" value="${vo.no }">
						<textarea rows="3" cols="90" name="msg" style="float: left"></textarea> &nbsp;
						<input type=submit value="댓글쓰기" class="btn btn-sm btn-danger" style="height: 60px">
					</form>
				</td>
			</tr>
		</table>
