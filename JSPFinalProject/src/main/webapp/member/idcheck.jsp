...
$(function(){
	$('#okBtn').hide(); // 확인버튼 안보이게 (아이디 중복 안될 때 보이도록 할거야)
	$('#idBtn').on('click', function(){
		let id = $('#id').val()
		if(id.trim()==" "){ //id 공백을 다 제거했는데도 공백이라면
			$('#id').focus();
			return;
		}
		...
		/*
			HTML : 정적(화면만 출력)
			JavaScript : 브라우저를 동적으로 변경
			---------------------------------브라우저에서만 사용가능
			Java
				= Java : 오라클 연결
				= Servlet : 브라우저 연결
			
			JSP ===> Controller ===> Model(.do) ===> DAO
				<===			<===		<===
			.do : Model을 호출
		*/
		$.ajax({
			...
			success: function(result){ // 콜백 : 시스템에 의해 자동 호출
				let res = Number(result.trim()); // 정수형으로 변경
				if(res===0){
