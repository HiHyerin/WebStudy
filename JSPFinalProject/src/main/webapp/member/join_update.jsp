...
$(function(){
	...
	$('#eBtn').click(function(){
		...
		$.ajax({
			...
			success:function(result){
				...
				if(count==0){ //존재하지 x 이메일
					$('#ePrint').text(email +"는(은) 사용 가능한 이메일입니다.");
					$('#email').prop('readonly',true) // 비활성화 <input readonly>
					// prop('disabled', true), prop('checked', true)
				}else{
					$('#ePrint').text(email+"는(은) 이미 사용중인 이메일 입니다.");
					$('#email').val("");
					$('#email').focus();
				}
			}
		...
		$.ajax({
			...
			success:function(result){
				...
				if(count==0){ //존재하지 x 이메일
					...
					$('#tel1').prop('readonly',true) // 비활성화 <input readonly>
					// prop('disabled', true), prop('checked', true)
				}else{
					$('#tPrint').text(phone+"는(은) 이미 사용중인 번호 입니다.");
					$('#tel2').val("");
					$('#tel2').focus();
				}
			}
	...
	$('#joinBtn').click(function(){
		...
		//$('#join_frm').submit();
		let formData = $('#join_frm').serialize(); /* 버튼을 누르면 모든 입력창이 머시기? */
		$.ajax({
			type:'post',
