...
public class FoodDAO {
	...
	// 톰캣에 의해 미리 생성된 Connection을 가지고 온다 => MyBatis(DBCP, JPA)
	public void getConnection() {
		try {
			Context init = new InitialContext(); //POOL
			Context c = (Context)init.lookup("java://comp/env"); // lookup("찾을주소")
	...
	// connection 사용 후 반환(Connection 관리 영역 : POOL)
...
// 검색
	...
	public FoodVO foodDetailData(int fno) {
		...
		try {
			getConnection();
			String sql = "select fno, name, score, poster, type, parking, menu, address, tel "
					+"from food_location "
					+"where fno=?";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, fno);
